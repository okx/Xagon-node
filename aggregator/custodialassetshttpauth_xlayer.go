...
func (a *Aggregator) auth(ctx context.Context, req *http.Request) error {
	if a.cfg.CustodialAssets.SecretKey == "" || a.cfg.CustodialAssets.AccessKey == "" {
		return nil
	}
	req.Header[headerAccessKey] = []string{a.cfg.CustodialAssets.AccessKey}
	// Generate the signature
	signature, err := a.genAuth(ctx, req, algoSha256)
	if err != nil {
...
func (a *Aggregator) genAuth(ctx context.Context, req *http.Request, algorithm string) (string, error) {
	...
	return a.generateSignature(ctx, treeMap, body.String(), algorithm)
}

func (a *Aggregator) generateSignature(ctx context.Context, treeMap map[string][]string, body, algorithm string) (string, error) {
	...
	return encryptAES(hashString, a.cfg.CustodialAssets.SecretKey)
}
