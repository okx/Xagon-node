...
func NewWorker(constraints state.BatchConstraintsCfg, readyTxsCond *timeoutCond) *Worker {
	w := Worker{
...
func (w *Worker) AddTxTracker(ctx context.Context, tx *TxTracker, nonce uint64) (replacedTx *TxTracker, dropReason error) {
	w.workerMutex.Lock()
	...
	if !found {
		...
		addr = newAddrQueue(tx.From, nonce, balance)

		// Lock again the worker
		w.workerMutex.Lock()

		w.pool[tx.FromStr] = addr
		log.Debugf("new addrQueue %s created (nonce: %d, balance: %s)", tx.FromStr, nonce, balance.String())
	}
...
func (w *Worker) GetBestFittingTx(resources state.BatchResources) (*TxTracker, error) {
	...
	if w.txSortedList.len() == 0 {
		return nil, sequencer.ErrTransactionsListEmpty
	}
	...
	if foundAt != -1 {
		...
		return nil, sequencer.ErrNoFittingTransaction
	}
}
